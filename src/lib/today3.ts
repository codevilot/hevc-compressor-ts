...
function interPrediction(
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      predicted[y][x] = currentBlock[y][x] - referenceBlock[y][x];
    }
  }
...
function encode(frame: ImageData, previousFrame?: ImageData): Float32Array {
  ...
  for (const ctu of ctus) {
    ...
    if (previousFrame) {
      ...
      const intraEnergy = residualY.flat().reduce((a, b) => a + b * b, 0);
      const interEnergy = interResidualY.flat().reduce((a, b) => a + b * b, 0);
    ...
    const quantizationFactorY = 5;
    const quantizationFactorC = 10;
